...
docbook_format_line([#op{name=Name,
    ...
    docbook_format_line(Prev) ++
        ...
        "|" ++ strip(strip_todo(Doc)) ++ "\n";
docbook_format_line([]) ->
    "=== Generic Instructions\n" ++ 
    "[options=" ++ [$"] ++ "header" ++ [$"] ++ ",cols=\"30,9,9,32,51\"]\n"
        "|=================================================\n" ++
...
%% The upstream genop.tab sometimes carries internal "TODO:" notes in
%% its doc strings. Drop everything from "TODO:" onward so the notes do
%% not end up in the printed appendix.
strip_todo(Doc) ->
    case string:str(Doc, "TODO:") of
        0 -> Doc;
        Pos -> trim_right(string:substr(Doc, 1, Pos - 1))
    end.

trim_right(S) ->
    lists:reverse(
      lists:dropwhile(fun(C) -> C =:= $\s orelse C =:= $\n end,
                      lists:reverse(S))).
