...
Now we can call this function from Erlang syncronously, or send
...
```
That worked as expected; we did a syncronous call and immediately
got a response. Now let's try the asyncronous call:
