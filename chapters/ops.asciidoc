...
In Elixir we can connect the nodes by running the command `Node.connect(name)`.
...
coloring and the size of the history.
See hexdocs for full documentation. footnote:source-ops-iex-html-module-the-iex-exs-file[https://hexdocs.pm/iex/IEx.html#module-the-iex-exs-file]
...
See the Erlang Shell manual footnote:source-ops-shell-html[http://erlang.org/doc/man/shell.html]
...
documentation for the ssh module footnote:source-ops-ssh-html[http://erlang.org/doc/man/ssh.html] explains
...
NOTE: The `system_dir` defaults to `/etc/ssh`, but those keys are only readable to the root user, which is why we create our own in this example.
...
in the next chapter (See xref:CH-Processes[]).

You can also get information about the memory and
the memory allocators in the node through `i [enter]`.
In xref:CH-Memory[] we will look at how to decipher this information.
...
erts/emulator/beam/break.c footnote:source-ops-break-c-master[https://github.com/erlang/otp/blob/master/erts/emulator/beam/break.c].
