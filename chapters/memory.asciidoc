...
at: erts_alloc footnote:source-memory-erts-alloc-html[http://www.erlang.org/doc/man/erts_alloc.html]

All these allocators also come with a number of parameters that
...
The erts_alloc manual goes as far as to give the following warning footnote:source-memory-erts-alloc-html[]:

[quote, Ericsson AB, erts_alloc]
____
...
"Troubleshooting
Down the Logplex Rabbit Hole" footnote:source-memory-logplex-down-the-rabbit-hole[https://blog.heroku.com/archives/2013/11/7/logplex-down-the-rabbit-hole].
...
All specific allocators use either sys_alloc or mseg_alloc to
...
allocations there is also a _relative max cache bad fit_ value which
...
In an SMP system there is usually one allocator of each type per
scheduler thread.
...
in a singleblock carrier, which as the name implies contains one
block.
...
the following allocation strategies footnote:source-memory-erts-alloc-html[]:
...
by temp_alloc may not be allocated over an Erlang process context
switch.
...
has died. If the reference count of the refc binary
...
processes that have seen a binary need to do a GC.
...
pattern match on a binary and getting a sub binary to work on is
what you want.

There are some degenerate cases, imagine for example that you load a
...
Constructing and Matching Binaries footnote:source-memory-binaryhandling-html[http://www.erlang.org/doc/efficiency_guide/binaryhandling.html].
