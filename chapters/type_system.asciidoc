...
==== Lists on 64-Bit Machines  
On 64-bit systems each cons cell is two full words (one for the head pointer, one for the tail pointer), aligned to 8 bytes, with no separate header. The list tag in the low two bits of each word still identifies the cell as a cons. Because there’s no header word per cell, Erlang gains a 33% space savings versus a boxed tuple, at the cost of one extra pointer dereference per element.  
