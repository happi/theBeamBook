...
execution in the Erlang VM. In the interpreter, the way the C code
...
the next process to execute. When running with the JIT (see
xref:CH-JIT[]), `process_main` is a global code fragment that serves
the same role: it picks the next process, restores its registers, and
jumps to the saved instruction pointer in native code.
