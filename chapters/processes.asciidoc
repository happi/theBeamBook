...
or from the Erlang shell with `observer:start()`.
...
[NOTE]  If you are building your application with erlang.mk or
...
the Erlang tagging scheme (which we will go through in <ref
linkend="ch.types"/> and about the execution model and error handling
which we will go through in <ref linkend="ch.BEAM"/>), but using these
...
These BIFs examines the internals of ERTS in a ways that
...
Theses BIFs are only meant to be used for debugging
...
in the code section and the instructions in <ref linkend="ap.building_erts"/>.
...
at what this fields in the PCB means.

=== The PCB

The Process Control Block contains all the fields that control the
behavior and current state of a process. In this section and the rest
...
this chapter, instead we will cover those in <ref linkend="ch.BEAM"/>.
...
[`erl_process.h`.](https://github.com/erlang/otp/blob/OTP_R16B03-1/erts/emulator/beam/erl_process.h)
...
The process id is an Erlang term and hence tagged. (See <ref
linkend="sec.tagging"/>) This means
...
The GC allocates a new memory area called _to space_.
...
`old_htop` and `old_hend` in the PCB. This almost bings us back to
...
See <ref linkend="ch.memory"/> for more
...
<filename>erl_message.h</filename>) appends the message to the receiver's
message queue.
...
old heap to the new heap. (In <ref linkend="ch.memory"/> we will
see the details of how garbage collection works.)

=== Dig In

In this chapter we have looked at how a process in implemented. In
